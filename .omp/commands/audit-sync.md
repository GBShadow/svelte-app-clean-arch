---
description: Detecta drift entre código e docs — varre rotas, estrutura, features/specs/workflow e débitos vs CHANGELOG/CODE-STRUCTURE/ROUTES/TECH-DEBT/LESSONS-LEARNED. Toca o stamp lido pelo hook. Sem args.
---
...
Ao final, execute `touch .omp/.audit-sync-stamp`. O hook (`session-changes.ts`) lê esse stamp — se passar de 24h sem `/audit-sync` em sessão que edite `pocketbase/pb_migrations/`, a IA é alertada antes de prosseguir. Sem o `touch`, o aviso fica permanente.
