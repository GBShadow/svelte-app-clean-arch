...
Persiste o estado da sessão em `docs/sessions/` e sincroniza a documentação versionada invocando o agente **`docs-writer`**, que opera sobre `.omp/.session-changes.log` (alimentado pelo hook) e `git diff`.
...
Despache o agente via task tool com `agent: "docs-writer"`.
...
O agente vai:
- Ler `.omp/.session-changes.log` + `git diff`
- Atualizar docs conforme mapeamento (abaixo)
- **Truncar** `.omp/.session-changes.log`
