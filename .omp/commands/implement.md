...
Conduz o ciclo completo de uma feature no svelte-app, do spec ao commit-ready, seguindo o fluxo spec-driven (skill `spec-driven`). **Pause e gate explícito ao fim de cada fase** — o ciclo só avança quando o gate da fase passa. `gh pr create` é **sugerido**, nunca executado (R13); commit/push é sempre decisão do usuário.
...
1. Invocar `spec-creator` via task tool (`agent: "spec-creator"`):
   - Seguir a skill `spec-driven` — ler `docs/CODE-STRUCTURE.md`, as skills `pocketbase-collections` e `pocketbase-api-rules` (regras do banco) e analisar segurança (XSS, realtime, IDOR).
...
1. Invocar `spec-reviewer` via task tool (`agent: "spec-reviewer"`, modo checklist):
...
1. Invocar `spec-reviewer` via task tool (`agent: "spec-reviewer"`, modo analyze, **read-only**):
...
1. Invocar `spec-converge` via task tool (`agent: "spec-converge"`):
