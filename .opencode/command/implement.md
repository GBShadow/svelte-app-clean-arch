...
Conduz o ciclo completo de uma feature no svelte-app, do spec ao commit-ready, seguindo o fluxo spec-driven (`.agents/skills/spec-driven.md` / `.opencode/skills/spec-driven/SKILL.md`). **Pause após cada fase** para o usuário aprovar antes da próxima.
...
1. Invocar `spec-creator` via Task tool (`subagent_type: "spec-creator"`):
   ...
   - Instruir: seguir `.opencode/skills/spec-driven/SKILL.md` — ler `docs/CODE-STRUCTURE.md`, as skills `pocketbase-collections` e `pocketbase-api-rules` (regras do banco), analisar segurança (XSS, realtime, IDOR) e gerar `docs/specs/<slug>.md` + `docs/workflow/<slug>.jira.md`.
2. Após a spec, invocar `spec-reviewer` via Task tool (`subagent_type: "spec-reviewer"`) para validar completude, consistência com Ports & Adapters e AC testáveis.
...
1. Determinar escopo:
   ...
   - Infra/Docker/CI → reportar ao usuário (tratar manualmente).
...
1. Invocar `test-writer` via Task tool (`subagent_type: "test-writer"`):
...
1. Invocar `code-reviewer` via Task tool (`subagent_type: "code-reviewer"`):
...
1. Invocar `docs-writer` via Task tool (`subagent_type: "docs-writer"`):
   - Lê `.opencode/.session-changes.log` + `git diff` + a spec.
   ...
   - **Trunca** `.opencode/.session-changes.log`.
