...
test.describe('Todo List (runes e2e)', () => {
	test('carrega todos iniciais e percentual de conclusão', async ({ page }) => {
		...
		await expect(page.getByText('Svelte 5 Runes')).toBeVisible();
		...
		await expect(page.getByText(/^\d+%$/)).toHaveText('33%');
	});

	test('adiciona um item via input', async ({ page }) => {
		await page.goto('/');

		await page.getByPlaceholder('Add a new todo...').fill('Nova tarefa e2e');
		await page.getByRole('button', { name: 'Add' }).click();

		await expect(page.getByText('Nova tarefa e2e')).toBeVisible();
		await expect(page.getByText(/^\d+%$/)).toHaveText('25%');
	});
	...
	test('marca item como done e remove item', async ({ page }) => {
		...
		await item.locator('..').getByRole('checkbox').click();
