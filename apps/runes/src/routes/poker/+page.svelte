					...
					<div class="card-body p-6 flex flex-col justify-between h-full">
						<div>
							...
							<div class="flex items-center gap-2 text-xs text-base-content/50 mb-6">
								...
								<span>Criador: {room.expand?.created_by?.name || 'Admin'}</span>
							</div>
						</div>
						<div class="card-actions justify-end">
							<a
								href="/poker/{room.id}"
								class="btn btn-primary btn-sm btn-outline group"
								data-testid="btn-enter-room-{room.id}"
							>
								Entrar na sala
			...
			>
				<div class="form-control w-full">
					...
					<input
						...
						name="name"
						bind:value={name}
						placeholder="Ex: Scrum Sprint 15"
				...
				<div class="modal-action flex justify-end gap-2 mt-6">
					...
					<button
						type="submit"
						class="btn btn-primary"
						data-testid="btn-submit-create-room"
					>
						Criar Sala
					</button>
				</div>
