...
- **Sem `ChatGateway`/`ChatMemoryGateway`/`ChatPocketBaseGateway`**: a spec já registrava essa alternativa descartada (ver "Alternativas consideradas" em `docs/specs/chat-realtime.md`) — o app `runes` não usa a abstração `Gateway` de `packages/todo-domain` para features PocketBase; seguido o mesmo padrão de `pocketbase-todo-sharing` (`locals.pb` direto + funções puras de autorização testadas isoladamente).
