...
- Dupla validação de participação (API Rules do PocketBase + checagem server-side no SvelteKit), seguindo o padrão de [`pocketbase-todo-sharing`](./2026-07-09-pocketbase-todo-sharing.md)
...
- Push notifications de mensagem nova — coberto pela spec separada [`chat-notifications`](./chat-2026-07-15-notifications.md)
...
- Hoje nenhuma coleção permite que um usuário comum liste outros usuários (`user.listRule`/`viewRule` e `auth.listRule`/`viewRule` são "próprio registro ou admin" — ver `0005_user_auth_rules.js`), o que inviabilizaria o RF4 (escolher participantes) para não-admins. Uma nova migration abre `user.listRule`/`viewRule` para `@request.auth.id != ''` (qualquer autenticado lista nome/e-mail/cargo de todos os usuários); a coleção `auth` permanece restrita (não expõe `isAdmin`/`mustChangePassword`/`passwordSetAt` de outros usuários). Ao criar a sala (ou adicionar participante depois), o servidor resolve o e-mail de cada participante escolhido para o `id` correspondente na coleção `auth`. **Isso não pode ser feito com o `locals.pb` da sessão do usuário comum** (ele só enxerga o próprio registro `auth`, mesmo padrão de posse já usado em `findAuthRecordByUserEmail` de `users/[id]/edit/+page.server.ts`, que só funciona porque quem chama lá é sempre admin) — descoberto durante a implementação da Task 10. A resolução usa em vez disso um cliente PocketBase autenticado como **superusuário** (`pocketbaseAdmin.ts`, ver Design), criado só para essa consulta pontual, sem reabrir `auth.listRule`/`viewRule` para usuários comuns.
...
| Server (lookup) | `apps/runes/src/lib/server/authLookup.ts` (`findAuthRecordByEmail(email)`, extraído do padrão hoje duplicado localmente em `users/[id]/edit/+page.server.ts`, reutilizado para resolver participantes escolhidos por e-mail para IDs de `auth`) — usa internamente um cliente PocketBase **superusuário** (`pocketbaseAdmin.ts`, ver linha abaixo), não o `locals.pb` da sessão do usuário, porque `auth.listRule`/`viewRule` continuam restritas a "próprio registro ou admin" e um usuário comum não consegue enxergar o registro `auth` de outro usuário para resolver o e-mail |
| Server (admin) | `apps/runes/src/lib/server/pocketbaseAdmin.ts` (novo: `getAdminClient()` — client PocketBase autenticado como superusuário via `_superusers`, usando `PB_ADMIN_EMAIL`/`PB_ADMIN_PASSWORD` já existentes em `.env`, cacheado em memória; usado só server-side, nunca exposto ao client; só o `.id` resolvido sai de `authLookup.ts`, nunca o registro `auth` completo) |
...
- Jira (após aprovação da spec): `docs/workflow/2026-07-10-chat-realtime.jira.md`
- Feature doc (pós-implementação): `docs/features/2026-07-10-chat-realtime.md`
- PR: `docs/workflow/2026-07-11-chat-realtime.pr.md`
- Depende de: [`pocketbase-auth`](./2026-07-09-pocketbase-auth.md), [`pocketbase-user-crud`](./2026-07-09-pocketbase-user-crud.md)
- Specs relacionadas: [`chat-notifications`](./chat-2026-07-15-notifications.md) (consome o realtime desta spec)
