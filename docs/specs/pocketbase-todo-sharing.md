...
Hoje `apps/runes` tem uma única lista de todo global, sem dono. Com autenticação e usuários já existentes ([`pocketbase-auth`](./pocketbase-auth.md), [`pocketbase-user-crud`](./pocketbase-user-crud.md)), cada usuário deve poder ter várias listas próprias, e opcionalmente compartilhar uma lista (somente leitura) com qualquer outro usuário autenticado que tenha o link, via uma flag `public`.
...
- Jira (após aprovação da spec): `docs/workflow/pocketbase-todo-sharing.jira.md`
- Feature doc (pós-implementação): `docs/features/pocketbase-todo-sharing.md`
- PR: `docs/workflow/pocketbase-todo-sharing.pr.md`
- Depende de: [`pocketbase-infra`](./pocketbase-infra.md), [`pocketbase-auth`](./pocketbase-auth.md)
- Specs relacionadas: [`pocketbase-user-crud`](./pocketbase-user-crud.md)
